...
func TestConnectionTarget(t *testing.T) {
	tests := []struct {
		...
		{name: "serial", cfg: config.ConnectionConfig{Transport: config.TransportSerial, SerialPort: "/dev/ttyACM0", SerialBaud: 115200}, want: "/dev/ttyACM0@115200"},
		...
		{name: "bluetooth with adapter", cfg: config.ConnectionConfig{Transport: config.TransportBluetooth, BluetoothAddress: "AA:BB:CC:DD:EE:FF", BluetoothAdapter: "hci1"}, want: "AA:BB:CC:DD:EE:FF (hci1)"},
	}
